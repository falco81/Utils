#!/usr/bin/env python3
"""
video_tools.py  (Windows 10 CLI version, no alass)
=====================================================
...
2) pip install numpy colorama charset-normalizer deep-translator
   (the complete dependency list incl. optional ones and installation on
    Linux/AlmaLinux is in `python video_tools.py --help`)
...
Usage
-----
    python video_tools.py --list-tracks video.mkv

    python video_tools.py video.mkv subtitles_cz.srt output_cz_synced.srt

    python video_tools.py video.mkv subtitles_cz.srt output.srt --ref-lang eng

    python video_tools.py video.mkv subtitles_cz.srt output.srt --track-id 2

    # sync by the audio track only (speech detection), without subtitle reference
    python video_tools.py video.mkv subtitles_cz.srt output.srt --audio-mode replace

    # combine subtitle reference + audio analysis for max accuracy
    python video_tools.py video.mkv subtitles_cz.srt output.srt --audio-mode combine --audio-lang cze
"""
...
# (e.g. /mnt/nas/video_tools). Priority for LOADING: --config-url / CONFIG_URL (a
# remote JSON, primary) > --config-file (one-off path) > this constant >
# the 'video_tools.configpath' pointer file (set via --config) > default.
CONFIG_STORE_PATH = r""

# Optional REMOTE config: point this at a JSON settings file (e.g. on your NAS) and
# it becomes the PRIMARY source of the config at startup - fetched read-only over
# HTTP(S). If empty, unreachable, or invalid, the tool silently falls back to the
# local path/pointer/default below. Can also be given at runtime with --config-url.
# Saving (via --config) always writes to the LOCAL file, not the URL.
CONFIG_URL = r"http://nas.falco81.net/video_tools.config.json"   # e.g. "http://nas.falco81.net/video_tools.config.json"
CONFIG_FETCH_TIMEOUT = 4          # seconds - kept short so an unreachable URL can't stall startup
...
def _config_url():
    return (_STORE_URL or CONFIG_URL or "").strip()
...
def _fetch_config_url():
    ...
    try:
        import urllib.request
        req = urllib.request.Request(url, headers={"User-Agent": "video_tools"})
...
def current_store_path():
    ...
    if n:
        return n
    # 4) default: next to the script (falls back to the old video_tool.config.json name)
    p = default_store_path()
    if not os.path.exists(p):
        legacy = os.path.join(os.path.dirname(p), "video_tool.config.json")
...
def save_store(store):
    ...
    try:
        ...
        try:
            ...
            pass
        if _config_url():
            log_warn(f"Note: a remote config URL is active - it is loaded FIRST at startup, so these "
                     f"local changes (saved to {os.path.basename(p)}) may be overridden by the URL. "
                     f"Update the remote JSON to change what's actually used.")
    except Exception as e:
...
def migrate_legacy_store():
    ...
    the new video_tools.config.json (when the new one doesn't exist yet)."""
...
class OpenSubtitles:
    ...
    def __init__(self, api_key, ua="video_tools v1.0", username=None, password=None):
        self.key = api_key
        self.ua = ua
        self.user = username
...
def _config_location_wizard(args):
    ...
    log_info("  Linux:   /mnt/nas/video_tools (e.g. an SMB/CIFS mount), ~/...")
...
def process_single(args):
    ...
    if args.list_tracks or not args.subtitle_to_fix or not args.output:
        ...
        if not args.list_tracks:
            print("\nUsage: python video_tools.py video.mkv subtitles.srt output.srt [--ref-lang eng] [--audio-mode combine]")
        return
...
def _wizard_action_specs():
    ...
    return {
        ...
        "config": dict(
            label="Set API keys and default options (video_tools.config.json)",
            help="Saves API keys and default options into video_tools.config.json (loaded automatically).",
...
SIMPLEST - the interactive wizard (the script asks about EVERYTHING step by step):
    python video_tools.py --auto                 # single file, searches this directory
    python video_tools.py --auto D:\folder       # ... in the given directory
    python video_tools.py --auto-all D:\series    # whole batch, but with questions up front
...
TRANSLATE SUBTITLES INTO ANOTHER LANGUAGE (extract from a video and save .srt):
    python video_tools.py --translate-subs D:\series
...
STEP 2 - align a single file (the most common case):
    python video_tools.py video.mkv subtitles_cz.srt output_cz.srt
  ...
  - No usable subtitle reference (only image-based PGS, or none)?
        python video_tools.py video.mkv t_cz.srt out.srt --audio-mode replace
...
STEP 3 - check the result in a player. When it almost fits, but some scenes "drift"
  differently from the rest = piecewise desync -> force the content-based method:
    python video_tools.py video.mkv t_cz.srt out.srt --method warp
...
BATCH (a whole series in one directory) - videos are paired with .srt by name:
    python video_tools.py D:\series --all --target-lang cs --overwrite
...
READABILITY ONLY, without synchronization (the subtitles already have correct timing):
    python video_tools.py D:\series --fix-readability
...
"""),
    ...
    parser.add_argument("--config", action="store_true",
                         help="Interactive setup of API keys and default options into video_tools.config.json (asks only about what you want to enable). Loaded automatically at startup.")
    ...
    parser.add_argument("--config-url", default=None, help="PRIMARY config source: URL of a remote settings JSON "
                        "(e.g. on your NAS), fetched read-only at startup. Falls back to the local file if empty/"
                        "unreachable. Set permanently via the CONFIG_URL constant. Saving still writes locally.")
    ...
    parser.add_argument("--preset-file", default=None, help="(deprecated) Alias for --config-file; presets are now in video_tools.config.json.")
