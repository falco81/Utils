...
// ---- live activity -------------------------------------------------------
require_once __DIR__ . '/plexmon-api.php';

// Live now-playing + activity: the daemon already has both, fresh.
if (isset($_GET['live']) || isset($_GET['sessions'])) {
    plexmon_relay('/sessions');
    exit;
}

// Poster art: the daemon cached each poster next to data.json, so we just serve
// the file. Falls back to the daemon if for some reason it isn't there yet.
if (isset($_GET['art'])) {
    $key = (string) $_GET['art'];
    if (!preg_match('#^/library/[A-Za-z0-9/_.\-]+$#', $key)) { http_response_code(400); exit; }
    $file = __DIR__ . '/art-' . substr(md5($key), 0, 16) . '.jpg';
    if (is_file($file)) {
    ...
    // Not cached yet — ask the daemon to fetch it live. This is what makes the
    // poster show up immediately instead of only after a collection pass.
    [$status, $body] = plexmon_call('/art?key=' . rawurlencode($key));
    if ($status === 200 && strlen($body) > 200) {
        header('Content-Type: image/jpeg');
        header('Cache-Control: private, max-age=600');
        echo $body;
        exit;
...
if (isset($_GET['wake'])) {
    plexmon_relay('/wake', 'POST', true);
    exit;
}
if (isset($_GET['wakecheck'])) {
    plexmon_relay('/wakecheck');
    exit;
}
...
// API directly — everything below is a plain read of the snapshot file.)
$data = is_readable(DATA_FILE)
...
function temp_class(?int $t, array $thr, array $d = []): string {
    ...
    // NVMe media is rated far hotter than a spinning disk.
    // The daemon resolves this per disk (drive's own limits, then NVMe
    // defaults, then the global ones) so both sides agree on the line.
    $crit = $d['temp_crit_eff'] ?? $d['temp_crit_own'] ?? ($thr['temp_crit'] ?? 58);
    $warn = $d['temp_warn_eff'] ?? $d['temp_warn_own'] ?? ($thr['temp_warn'] ?? 50);
...
function disk_state(array $d): array {
    ...
    $io    = $d['io'] ?? 'idle';
    // a wake request is spinning this disk up right now
    if ($power === 'spinning') return ['Spinning up', 'read', false];
...
?>
  ...
  <section class="panel" id="tab-overview">
    ...
    <?php foreach ($data['disks'] as $d):
        ...
        $tclass = temp_class($d['temp'], $thr, $d);
        $pct = $d['fs_pct'];
    ...
    ?>
      <div class="card disk<?= $asleep ? ' asleep' : '' ?>">
        ...
        <div class="temp">
          ...
          <?php if (!empty($d['smart_stale'])): ?><span class="cachenote">cached<?= ($d['cache_age'] ?? null) !== null ? ' ' . h(human_uptime((int) $d['cache_age'])) . ' ago' : '' ?></span>
